...
import { ArrowUpRight, Menu, X, Play, Phone, Mail, MessageCircle, ChevronRight } from 'lucide-react'
    ...
    <>
      ...
      >
        <a href="/" className="flex items-center gap-2 group select-none">
          <span className="text-[10px] tracking-widest transition-colors duration-300 text-purple-400/60 group-hover:text-purple-400">&#10022;</span>
          <span className="text-[13px] font-medium tracking-[0.14em] lowercase text-white/90">theyashthetics</span>
        </a>

        <ul className="hidden lg:flex items-center gap-10">
          {links.map((l) => (
            <li key={l.label}>
              <a href={l.href} className="text-[13px] font-normal transition-colors duration-300 tracking-wide text-white/40 hover:text-white">
                {l.label}
              </a>
            </li>
          ))}
        ...
        <a
          ...
          className="hidden lg:flex items-center gap-1.5 text-[12px] font-normal tracking-[0.06em] border-b pb-0.5 transition-colors duration-300 group text-white/90 border-white/30 hover:border-purple-400"
        >
          Let&apos;s work
          <ArrowUpRight size={12} className="group-hover:translate-x-0.5 group-hover:-translate-y-0.5 transition-transform duration-300" />
        </a>

        <button onClick={() => setMenuOpen(true)} className="lg:hidden p-1 text-white" aria-label="Open menu">
          <Menu size={19} />
        </button>
      </motion.nav>
      ...
      <AnimatePresence>
        {menuOpen && (
          <motion.div
            ...
            className="fixed inset-0 z-[100] bg-[#0a0a0f] flex flex-col px-8 py-6"
          >
            <div className="flex justify-between items-center">
              <span className="text-[13px] font-medium tracking-[0.14em] text-white/90 lowercase">theyashthetics</span>
              <button onClick={() => setMenuOpen(false)} className="p-1 text-white"><X size={19} /></button>
            </div>
            <ul className="flex flex-col gap-6 mt-16">
              {links.map((l, i) => (
                ...
                >
                  <a href={l.href} onClick={() => setMenuOpen(false)} className="text-4xl font-light text-white tracking-tight hover:text-purple-400 transition-colors duration-200">
                    {l.label}
                  </a>
                </motion.li>
              ))}
            ...
            <div className="mt-auto flex flex-col gap-2">
              <a href="mailto:iamyash.200@gmail.com" className="text-sm text-white/40 tracking-wide">iamyash.200@gmail.com</a>
              <a href="tel:+918533017236" className="text-sm text-white/40 tracking-wide">+91 85330 17236</a>
              <span className="text-[10px] text-white/20 tracking-widest uppercase mt-2">2025 &copy; theyashthetics</span>
            </div>
          </motion.div>
        )}
      </AnimatePresence>
...
function HeroBackground() {
  return (
    <div className="absolute inset-0 z-0 overflow-hidden w-full h-full">
      ...
      <div className="absolute inset-0 z-10 bg-gradient-to-r from-[#0a0a0f]/90 via-[#0a0a0f]/50 to-transparent" />
      <div className="absolute inset-0 z-10 bg-gradient-to-b from-purple-950/30 via-transparent to-[#0a0a0f]/40" />
      <div className="absolute inset-0 z-10 bg-[#0a0a0f]/55 lg:hidden" />
    </div>
  )
}
    ...
    <div className="w-full py-8 md:py-12">
      <div className="max-w-7xl mx-auto px-6 w-full">
        ...
        <motion.div {...fadeUp(0.2)} className="flex items-center gap-2.5 mb-8">
          ...
          <span className="text-[10px] uppercase tracking-[0.24em] text-purple-400/70 font-light">Video Editor &middot; theyashthetics</span>
        </motion.div>

        <h1 className="text-4xl sm:text-5xl md:text-6xl lg:text-[72px] font-bold tracking-[-0.03em] leading-[1.08] text-white mb-8 whitespace-pre-line min-h-[2.2em] font-heading">
          {displayed}
          {!done && <span className="animate-blink font-extralight text-purple-400/60 ml-0.5">|</span>}
        </h1>

        <motion.p {...fadeUp(1.8)} className="text-[15px] sm:text-[17px] text-white/50 leading-relaxed max-w-[520px] mb-10 font-light">
          {subtitle}
        </motion.p>
        ...
        <motion.div {...fadeUp(2.2)} className="flex flex-wrap items-center gap-4">
          <a
            href="#work"
            className="inline-flex items-center gap-2 px-7 py-3.5 bg-gradient-to-r from-purple-500 to-pink-500 text-white text-[13px] font-medium tracking-wide rounded-full hover:shadow-[0_0_30px_rgba(168,85,247,0.4)] transition-all duration-300"
          >
            See Relevant Work
          ...
          <a
            ...
            className="inline-flex items-center gap-2 px-7 py-3.5 border border-white/20 text-white text-[13px] font-normal tracking-wide rounded-full hover:border-purple-400/50 hover:bg-white/5 transition-all duration-200"
          >
            Book a Call
...
function TrustStrip() {
  const clients = [
    { name: 'Pan Pacific Singapore', initials: 'PP', color: 'bg-purple-500/20 text-purple-300' },
    { name: 'Taj Amritsar', initials: 'TA', color: 'bg-pink-500/20 text-pink-300' },
    { name: 'Salim Merchant Event', initials: 'SM', color: 'bg-cyan-500/20 text-cyan-300' },
    { name: 'Media Scoop', initials: 'MS', color: 'bg-blue-500/20 text-blue-300' },
  ]
  ...
  return (
    <FadeIn>
      <section className="px-6 pt-12 pb-10 max-w-7xl mx-auto">
        <p className="section-label mb-6">Trusted By</p>
        ...
        <div className="flex flex-wrap items-center gap-6 md:gap-12 pt-6 border-t border-white/[0.06]">
          {indicators.map(({ label }) => (
            <div key={label} className="flex flex-col">
              <span className="text-[28px] font-light text-purple-400 tracking-tight leading-none mb-1">✓</span>
  ...
  return (
    <FadeIn>
      <section className="px-6 py-16 max-w-7xl mx-auto">
        <div className="bg-gradient-to-br from-white/[0.04] to-white/[0.02] border border-white/[0.06] rounded-2xl px-8 py-14 md:px-14 text-center backdrop-blur-sm">
          <p className="section-label mb-4">Free Offer</p>
          <h2 className="text-2xl md:text-3xl font-bold text-white tracking-tight mb-3 font-heading">
            Get a Free Video Breakdown
          </h2>
          <p className="text-[15px] text-white/40 font-light max-w-lg mx-auto mb-12 leading-relaxed">
          ...
          <div className="flex flex-col md:flex-row gap-6 md:gap-0 max-w-2xl mx-auto mb-12">
            {steps.map((s, i) => (
              <div key={s.num} className="flex-1 flex flex-col items-center text-center gap-3 relative">
                <div className="w-9 h-9 rounded-full bg-gradient-to-br from-purple-500 to-pink-500 text-white flex items-center justify-center text-[13px] font-semibold">
                  {s.num}
                </div>
                <p className="text-[14px] font-medium text-white">{s.title}</p>
                <p className="text-[12px] text-white/35 font-light leading-relaxed max-w-[200px]">{s.desc}</p>
                {i < steps.length - 1 && (
                  <div className="hidden md:block absolute top-4 left-[60%] w-[80%] h-px bg-white/[0.06]" />
                )}
              </div>
            ))}
          </div>
          ...
          <a
            ...
            className="inline-flex items-center gap-2 px-8 py-3.5 bg-gradient-to-r from-purple-500 to-pink-500 text-white text-[13px] font-medium tracking-wide rounded-full hover:shadow-[0_0_30px_rgba(168,85,247,0.4)] transition-all duration-300"
          >
...
// ─── Global video tracker (only one video plays at a time) ───────────────────
...
function CaseStudies() {
  ...
  return (
    <FadeIn>
      <section className="px-6 py-16 max-w-7xl mx-auto">
        <div className="mb-10 pb-4 border-b border-white/[0.06]">
          <h2 className="text-2xl md:text-3xl font-bold text-white tracking-tight font-heading">Why Clients Hire Me</h2>
        </div>
          ...
          {cases.map((c, i) => (
            <div key={i} className="case-card">
              <div className="mb-5">
                ...
                <p className="text-[14px] text-white/60 leading-relaxed font-light">{c.challenge}</p>
              </div>
              <div className="mb-5">
                <p className="case-label text-amber-400">What Changed</p>
                <p className="text-[14px] text-white/60 leading-relaxed font-light">{c.changed}</p>
              </div>
              <div>
                <p className="case-label text-emerald-400">Result</p>
                <p className="text-[14px] text-white/60 leading-relaxed font-light">{c.result}</p>
              </div>
            </div>
          ))}
        </div>
    ...
    <FadeIn>
      <section className="px-6 py-20 max-w-7xl mx-auto">
        <div className="text-center mb-14">
          <p className="section-label mb-3">REAL AUDIENCE REACTIONS</p>
          <h2 className="text-3xl md:text-4xl font-bold text-white tracking-tight mb-3 font-heading">Community Feedback</h2>
          <p className="text-[15px] text-white/40 font-light max-w-lg mx-auto">Real reactions from creators, audiences and collaborators.</p>
        </div>
        ...
        <p className="text-center text-[11px] text-white/30 font-light tracking-wide mt-10">
  ...
  return (
    <FadeIn>
      <section id="process" className="px-6 py-16 max-w-7xl mx-auto">
        <div className="mb-10 pb-4 border-b border-white/[0.06]">
          <h2 className="text-2xl md:text-3xl font-bold text-white tracking-tight mb-1 font-heading">How We Work</h2>
          <p className="text-[12px] text-white/40 font-light tracking-wide">Simple process. Premium results.</p>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          {steps.map(s => (
            <div key={s.num} className="process-step">
              ...
              <p className="text-[15px] font-medium text-white/90 mb-2">{s.title}</p>
...
function Services() {
  ...
  return (
    <FadeIn>
      <section className="px-6 py-16 max-w-7xl mx-auto">
        <div className="mb-10 pb-4 border-b border-white/[0.06]">
          <h2 className="text-2xl md:text-3xl font-bold text-white tracking-tight font-heading">What I Edit</h2>
        </div>
...
function Footer() {
  ...
  return (
    <footer id="contact" className="border-t border-white/[0.06] bg-transparent px-6 py-20">
      <div className="max-w-7xl mx-auto">
        ...
        >
          <h2 className="text-3xl md:text-5xl lg:text-6xl font-bold text-white tracking-tight leading-[1.05] mb-4 max-w-2xl font-heading">
            Need content that actually gets watched?
          </h2>
          <p className="text-[16px] text-white/40 font-light mb-6 max-w-lg">Let&apos;s discuss your next project.</p>
          <p className="text-[12px] text-white/30 font-light tracking-wide mb-6">Average response time: within 24 hours</p>
          ...
          <div className="flex flex-wrap gap-4 mb-16">
            {contacts.map(({ icon: Icon, label, href }) => (
              <a
                ...
                className="flex items-center gap-3 px-5 py-3.5 rounded-xl border border-white/[0.08] bg-white/[0.03] hover:bg-purple-500/15 hover:text-white hover:border-purple-500/30 hover:-translate-y-1 hover:shadow-lg hover:shadow-purple-500/10 transition-all duration-300 group backdrop-blur-sm"
              >
                <Icon size={15} className="text-white/40 group-hover:text-purple-400 transition-colors duration-300" />
                <span className="text-[13px] font-normal text-white/60 group-hover:text-white transition-colors duration-300 tracking-wide">{label}</span>
              </a>
            ))}
          </div>
          ...
          <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-4 pt-8 border-t border-white/[0.06]">
            <div className="flex items-center gap-2">
              <span className="text-[10px] text-purple-400/50 tracking-widest">&#10022;</span>
              <span className="text-[11px] font-medium tracking-[0.14em] text-white/40 lowercase">theyashthetics</span>
            </div>
            <div className="flex items-center gap-6">
              <span className="text-[10px] text-white/20 tracking-widest uppercase">Video Editor</span>
              <span className="text-[10px] text-white/10 tracking-widest">|</span>
              <span className="text-[10px] text-white/20 tracking-widest uppercase">&copy; 2025</span>
            </div>
          </div>
        </motion.div>
      </div>
...
export default function App() {
  ...
  return (
    <div className="relative bg-[#0a0a0f] text-white font-sans selection:bg-purple-500/20 selection:text-white antialiased overflow-x-hidden">
      <Routes>
